...
class SQLiteCasette {
	constructor(options) {
		this.dbfile = (options || {}).file || 'db.sqlite';
		this.clean = options && !!options.clean;
	}
...
class Connection {
	constructor(tx, options) {
		...
		this.casette = new MySQLCasette(options);//new SQLiteCasette(options);

		this._conn = tx || this.casette.generateConnection();
	...
	async queryPlan(sql, opt_args) {
		return await this.query(this.casette.planSql(sql), opt_args);
	}
	...
	async queryPlanFromFile(path, opt_args) {
		...
		return await Promise.all(
			zipWith(sqls, opt_args)
				.map(sa => this.query(this.casette.planSql(sa[0]), sa[1])
					.then(records => ({sql: this.casette.planSql(sa[0]), records: records}))
				)
		);
	}
